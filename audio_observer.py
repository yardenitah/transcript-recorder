...
class PcmAudioObserver(IAudioFrameObserver):
    ...
    def _process_frame(self, name, frame):
        # לוג אגרסיבי כדי לראות אם נכנסנו
        if self.frame_count % 50 == 0:
            logger.debug(f"💣 [INTERNAL] Processing frame from {name}")

        try:
            self.frame_count += 1
            data = bytes(frame.buffer)  # המרת הבאפר ל-bytes
            is_silence = all(b == 0 for b in data)
    ...
    # שימוש ב-*args כדי לתפוס כל וריאציה של פרמטרים
    def on_playback_audio_frame(self, *args):
        # args[0] = usually local_user
        # args[-1] = usually the frame
        frame = args[-1]
        return self._process_frame("on_playback", frame)
    ...
    def on_mixed_audio_frame(self, *args):
        # זה הקולבק הכי חשוב ב-Headless
        logger.debug("🟣 [CALLBACK] Mixed Audio Triggered")
        frame = args[-1]
        return self._process_frame("on_mixed", frame)

    def on_playback_audio_frame_before_mixing(self, *args):
        # ננסה לחלץ את ה-UID אם אפשר, אבל לא ניפול אם אי אפשר
        try:
            uid = args[2]  # בדרך כלל ה-UID הוא הפרמטר השלישי
        except:
            uid = "unknown"

        logger.debug(f"🔥 [CALLBACK] BeforeMixing Triggered! UID={uid}")
        frame = args[-1]
        return self._process_frame(f"before_mixing_{uid}", frame)
